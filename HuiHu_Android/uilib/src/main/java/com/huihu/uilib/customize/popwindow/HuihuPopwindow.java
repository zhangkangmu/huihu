package com.huihu.uilib.customize.popwindow;
...
/**
 * create by ouyangjianfeng on 2019/4/15
 * description:
 */
public class HuihuPopwindow extends PopupWindow {
    ...
    private HuihuPopwindow.OnItemClickListener onItemClickListener;
    ...
    public HuihuPopwindow(Activity activity) {
        this(activity, activity.findViewById(android.R.id.content));
    }

    public HuihuPopwindow(Context context, View view) {
        super(context);
        setOutsideTouchable(true);
        setFocusable(true);
        setBackgroundDrawable(new BitmapDrawable());
//        view.setOnTouchListener(new HuihuPopwindow.MenuTouchListener());
        this.context = context;
        this.view = view;
        VERTICAL_OFFSET = DensityUtil.dip2px(context, 10);
        DEFAULT_MENU_WIDTH = DensityUtil.dip2px(context, 100);
        screenPoint = DensityUtil.getScreenMetrics(context);
    ...
    private void generateLayout(int itemWidth) {
        ...
        menuLayout.setBackgroundDrawable(ContextCompat.getDrawable(context, R.drawable.uilib_bg_shadow));
        menuLayout.setOrientation(LinearLayout.VERTICAL);
        int padding = DensityUtil.dip2px(context, 12);
        for (int i = 0; i < menuItemList.size(); i++) {
            TextView textView = new TextView(context);
            textView.setClickable(true);
            textView.setBackgroundDrawable(ContextCompat.getDrawable(context, R.drawable.uilib_selector_item));
            ...
            if (menuModel.getItemResId() != View.NO_ID) {
                ...
                textView.setCompoundDrawablePadding(DensityUtil.dip2px(context, 12));
                textView.setCompoundDrawables(drawable, null, null, null);
            }
            textView.setText(menuModel.getItem());
            if (onItemClickListener != null) {
                textView.setOnClickListener(new HuihuPopwindow.ItemOnClickListener(i));
            }
    ...
    public void setOnItemClickListener(final HuihuPopwindow.OnItemClickListener onItemClickListener) {
        this.onItemClickListener = onItemClickListener;
        if (onItemClickListener != null) {
            for (int i = 0; i < menuLayout.getChildCount(); i++) {
                View view = menuLayout.getChildAt(i);
                view.setOnClickListener(new HuihuPopwindow.ItemOnClickListener(i));
            }
        }
    }
